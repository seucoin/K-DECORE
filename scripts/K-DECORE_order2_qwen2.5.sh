...
# —— path —— #
DATA_ROOT="../data/KBQA_CL_2stages_order2_qwen2.5"
INFO_JSON="${DATA_ROOT}/dataset_info.json"
export INFO_JSON
BASE_MODEL_PATH="your_<Qwen2.5-7B-Instruct>_model_path"
EVAL_SCRIPT="../evaluate/evaluate.py"

SAVE_ROOT="../saves/KBQA_CL_2stages_order2_qwen2.5/${MODEL_NAME}/seq_lora"
CFG_ROOT="../config/KBQA_CL_2stages_order2_qwen2.5/${MODEL_NAME}/seq_lora"
...
pushd "${DATA_ROOT}/task1" >/dev/null && python compwebq_split_test.py   && popd >/dev/null
pushd "${DATA_ROOT}/task2" >/dev/null && python spider_split_test.py     && popd >/dev/null
pushd "${DATA_ROOT}/task3" >/dev/null && python mtop_split_test.py   && popd >/dev/null
pushd "${DATA_ROOT}/task4" >/dev/null && python grailqa_split_test.py && popd >/dev/null
